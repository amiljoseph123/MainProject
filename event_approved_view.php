	...
	<section id="content">
		...
		<main>
			...
			<div class="left">
				...
				$result = mysqli_query($con, "SELECT * FROM `appiontment` WHERE `status`='approved'") or die("error");
					if (isset($_POST['add'])) {
						$id = $_POST['app_id'];
						$result1=mysqli_query($con, "SELECT `email` FROM `appiontment` WHERE `id` = $id");
						$row = $result1->fetch_assoc();
						// $get_email = 'amiljoseph333@gmail.com';
						$get_email = $row['email'];
						$pswde="hello12345";
					...
					elseif (isset($_POST['completeButton'])) {
						$id = $_POST['app_id'];
						mysqli_query($con, "UPDATE `appiontment` SET `action`='completed' WHERE `id`=$id") or die("error");
						// You can perform additional actions if needed
					} elseif (isset($_POST['postponeButton'])) {
						// Handle postponed logic
						// ...

if (isset($_POST['postponeButton'])) {
    $id = $_POST['app_id'];
    mysqli_query($con, "UPDATE `appiontment` SET `action`='postponed' WHERE `id`=$id") or die("error");
    // You can perform additional actions if needed
}
...
<td>
    <?php if ($row['action'] == 'completed'): ?>
        <span class="text-success">Completed</span>
    <?php elseif ($row['action'] == 'postponed'): ?>
        <span class="text-info">Postponed</span>
    <?php else: ?>
        <form action="" method="post">
            <button type="submit" class="details-button completed-button" name="completeButton">Complete</button>
            <input type="hidden" name="app_id" value="<?php echo $id; ?>">
        </form>
		<br>
        <form action="" method="post">
            <button type="submit" class="details-button postponed-button" name="postponeButton">Postpone</button>
            <input type="hidden" name="app_id" value="<?php echo $id; ?>">
        </form>
     <?php endif; ?>
     </td></tr>
